...
import { Download, ExternalLink, CheckCircle, Eye } from "lucide-react";
...
const Payments = () => {
  ...
  /* =========================
     Load Payments (Backend pagination)
  ========================= */
  const loadPayments = async (page = 1, limit = 10) => {
    try {
      ...
      const res = await api.get("/payments/list.php", { params });

      setPayments(res.data.data || []);
      setPagination(
        res.data.pagination || { page, limit, total: 0, totalPages: 1 }
      );
      setStatsApi(res.data.stats || null);
    } catch (err) {
      console.error(err);
  ...
  ========================= */
  useEffect(() => {
    loadPayments(1, pagination.limit);
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      loadPayments(1, pagination.limit); // filter changed => reset to page 1
    }, 400);
  ...
  const markPaid = async (payment) => {
    ...
    try {
      ...
      // ✅ Your backend should accept invoice_id (best)
      // If you already made /payments/mark-paid.php expecting "payment_id"
      // then change this to payment_id: payment.id
      ...
      await loadPayments(pagination.page, pagination.limit);
      setSelected(null);
    } catch (err) {
      console.error(err);
  ...
  /* ✅ Stats values (from API) */
  const totalPaidRevenue = Number(statsApi?.total_paid_amount || 0);
  const paidCount = Number(statsApi?.paid || 0);
  const pendingCount = Number(statsApi?.pending || 0);
  const failedCount = Number(statsApi?.failed || 0);
    ...
    <div className="space-y-6 p-5">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
        ...
        <input
          ...
          placeholder="Search gym / invoice / txn / ref..."
          className="w-full sm:w-80 h-10 px-3 rounded-lg border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-indigo-500"
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          ...
          <p className="text-2xl font-semibold text-slate-900">{paidCount}</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <TablePagination
          ...
          onPageChange={(newPage) => loadPayments(newPage, pagination.limit)}
          onLimitChange={(newLimit) => loadPayments(1, newLimit)}
        />
      </div>
